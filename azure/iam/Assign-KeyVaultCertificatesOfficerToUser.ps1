<#
.SYNOPSIS
    Assigns the "Key Vault Certificates Officer" role to a user on a specific Key Vault.

.DESCRIPTION
    Resolves a target Key Vault and user, then creates an Azure RBAC role assignment
    for the built-in role "Key Vault Certificates Officer" at Key Vault scope.
    ...
    - -ConfigName  (loads Assign-KeyVaultCertificatesOfficerToUser.<Name>.json from the script dir)
...
.EXAMPLE
    .\Assign-KeyVaultCertificatesOfficerToUser.ps1 -ConfigName prod

.EXAMPLE
    .\Assign-KeyVaultCertificatesOfficerToUser.ps1 -KeyVaultName 'kv-prod-001' -Upn 'user@contoso.com'
...
$resolvedConfigPath = Resolve-ToolkitConfigPath `
    ...
    -Prefix       'Assign-KeyVaultCertificatesOfficerToUser'
...
Write-Host '=== Assign-KeyVaultCertificatesOfficerToUser ===' -ForegroundColor Cyan
